...
class NsfwDetect:
    ...
    def classify_clip_with_key_frames(self, key_frames, clip_path, kinesis_event):
        ...
        Args:
            ...
            clip_path (string): file path to downloaded image
        ...
        try:
            self.reset()
            self.logger.debug("[{}] Reading clip: {}".format(user_id, clip_path))

            to_be_moderated = False
            self.logger.debug("[{}] Preparing clip for model.".format(user_id))
            ...
            self.logger.debug("[{}] Moderating.".format(user_id))
            ...
            output_dict["NN_DURATION"] = round(time.time() - start_time, 3)
