...
#     else:
#         use_all_frames = True
#         return (use_all_frames, all_frames, fps, video_length, np.arange(len(all_frames)))
...
def get_interest_frames_from_video(
    ...
    try:
        ...
        for frame_i in range(length + 1):
            ...
            # if skip_seconds > 0 and frame_i % low_skip_seconds != 0:
            #     pass
            # else:
            #     low_found_similar = False
            #     for context_frame_i, context_frame in reversed(
            #         low_important_frames[-1 * similarity_context_n_frames :]
            #     ):
            #         if is_similar_frame(
            #             context_frame, current_frame, thresh=frame_similarity_threshold
            #         ):
            #             low_found_similar = True
            #             break

            #     if not low_found_similar:
            #         low_important_frames.append((frame_i, current_frame))
    ...
    # if len(low_important_frames) != 0:
    #     low_important_frames = [i[1][:, :, [2, 1, 0]] for i in low_important_frames]
    # else:
    #     low_important_frames = all_frames
