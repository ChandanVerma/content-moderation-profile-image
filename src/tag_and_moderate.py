...
#     probs, framethresh, model_class_index_to_class_name, lomotiftopk, frametopk
# ):
#     top_label_idxs = []
#     top_label_probs = []
#     for p in probs:
#         x = {i: v for i, v in enumerate(p)}
#         x = dict(sorted(x.items(), key=lambda item: item[1]))
#         top_label_idx = list(x.keys())[-frametopk:]

#         top_label_prob = list(x.values())[-frametopk:]

#         top_label_idxs += top_label_idx
#         top_label_probs += top_label_prob
#     top_label_idxs = np.array(top_label_idxs)
#     top_label_probs = np.array(top_label_probs)

#     subset = top_label_idxs[top_label_probs > framethresh]
#     subset_probs = np.array(top_label_probs[top_label_probs > framethresh])
#     subset_labels = [model_class_index_to_class_name[x] for x in subset]
#     subset_labels = np.array(
#         [model_class_to_lomotif_pri_category[x] for x in subset_labels]
#     )

#     subset_dict = {}
#     for u in list(set(subset_labels)):
#         subset_dict[u] = np.sum(subset_probs[subset_labels == u])
#     subset_dict = dict(sorted(subset_dict.items(), key=lambda item: item[1]))
#     top_labels = list(subset_dict.keys())[-lomotiftopk:]
#     top_labels = top_labels[::-1]

#     if len(top_labels) == 0:
#         # fall back strategy for primary category prediction
#         max_prob_index = [np.argmax(x, axis=0) for x in probs]
#         max_prob = [np.max(x, axis=0) for x in probs]
#         preds = [model_class_index_to_class_name[idx] for idx in max_prob_index]
#         preds = [model_class_to_lomotif_pri_category[x] for x in preds]
#         preds = np.array(preds)
#         mode, mode_count = stats.mode(preds)
#         uniq, count = np.unique(preds, return_counts=True)
#         mode_labels = uniq[count == mode_count]
#         mode_labels = list(mode_labels)[0:lomotiftopk]
...
class ContentTagAndModerate:
    ...
    def generate_coop_predictions(self, image_list, user_id):
        ...
        """
        try:
            self.logger.info("[{}] Generating CoOp predictions.".format(user_id))
            probs_list = []
            ...
            self.logger.info("[{}] CoOp predictions generated.".format(user_id))
    ...
    def generate_clip_predictions(self, image_list, pred_pri_cat, user_id):
        ...
        try:
            ...
            # clip predictions
            self.logger.info("[{}] Generating CLIP predictions.".format(user_id))
            torch.cuda.empty_cache()
            sec_probs_list = []
            ...
            self.logger.info("[{}] CLIP predictions generated.".format(user_id))
    ...
    def run_service_with_key_frames(self, key_frames_list, kinesis_event):
        ...
        try:
            ...
            logits, pred_pri_cat = self.generate_coop_predictions(
                key_frames_list, user_id
            )
            # pred_time = str(pd.Timestamp.utcnow())
            output_dict["COOP_DURATION"] = round(time.time() - start_time, 3)
            ...
            (
                ...
                top_nplusone_pri,
                top_nplusone_sec,
            ) = self.generate_clip_predictions(key_frames_list, pred_pri_cat, user_id)
            # pred_time = str(pd.Timestamp.utcnow())
            ...
            output_dict["CLIP_DURATION"] = round(time.time() - start_time, 3)
            ...
            output_dict["CLIP_DURATION"] = 0
            output_dict["COOP_DURATION"] = 0
            output_dict["CLIP_TO_BE_MODERATED"] = True
